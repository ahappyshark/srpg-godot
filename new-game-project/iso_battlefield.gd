...
@onready var player: ISO_Player = get_parent().get_node("Player")
...
func world_to_cell(world: Vector2) -> Vector2i:
	# Convert global mouse position into map cell coords (works for iso)
	var local := map.to_local(world)
	return map.local_to_map(local)

func cell_to_world(cell: Vector2i) -> Vector2:
	# Returns the *local* position of the cell in the map’s space (usually the cell origin)
	var local := map.map_to_local(cell)
	return map.to_global(local)

func is_in_bounds(cell: Vector2i) -> bool:
	# Use the TileMapLayer's used rect as the movement boundary.
...
func draw_diamond(center_global: Vector2, fill: Color, outline: Color, outline_w: float = 2.0) -> void:
	var c := to_local(center_global) # because _draw() is in local space
	var top    := c + Vector2(0, -TILE_H / 2.0)
	var right  := c + Vector2(TILE_W / 2.0, 0)
	var bottom := c + Vector2(0, TILE_H / 2.0)
	var left   := c + Vector2(-TILE_W / 2.0, 0)

	draw_colored_polygon([top, right, bottom, left], fill)
	draw_polyline([top, right, bottom, left, top], outline, outline_w)

func _draw() -> void:
	# Highlight reachable cells
	for c: Vector2i in valid_cells:
		var p: Vector2 = cell_to_world(c)
		draw_diamond(p, Color(0.2, 0.8, 1.0, 0.10), Color(0.2, 0.8, 1.0, 0.10), 1.0)

	# draw damage cells
	for key in damage.keys():
		var c: Vector2i = key
		var p: Vector2 = cell_to_world(c)
		draw_diamond(p, Color(1.0, 0.2, 0.2, 0.20), Color(1.0, 0.2, 0.2, 0.20), 1.0)		
	...
	for c: Vector2i in preview_path:
		...
		draw_diamond(p, fill, outline, 1.0)

	# draw blocked cells
	for key in blocked.keys():
		var c2: Vector2i = key
		var p2: Vector2 = cell_to_world(c2)
		draw_diamond(p2, Color(0.0, 0.0, 0.0, 0.35), Color(0.0, 0.0, 0.0, 0.8), 1.0)


	# Hover highlight
	if is_in_bounds(hover_cell):
		var p3: Vector2 = cell_to_world(hover_cell)
		draw_diamond(p3, Color(1, 1, 1, 0.12), Color(1, 1, 1, 0.5), 1.0)
